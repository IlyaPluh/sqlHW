...
where role_title like '%Automation QA%';
 ...
 select roles_employee.employee_id, employees.employee_name, roles.role_title, salary.monthly_salary
 from employees
 ...
 select employees.employee_name, salary.monthly_salary, roles.role_title
 from employees
 left join roles_employee on employees.id = roles_employee.employee_id 
 left join roles on roles.id = roles_employee.role_id 
 left join employee_salary on roles_employee.employee_id = employee_salary.employee_id 
...
where role_title like '%Python%' ;
 ...
 left join employee_salary on roles_employee.employee_id = employee_salary.employee_id 
 left join salary on employee_salary.salary_id = salary.id 
 where role_title like '%Junior Python%';
 ...
 left join employee_salary on roles_employee.employee_id = employee_salary.employee_id 
 left join salary on employee_salary.salary_id = salary.id 
 where role_title like '%Middle JavaScript%';
 ...
 left join employee_salary on roles_employee.employee_id = employee_salary.employee_id 
 left join salary on employee_salary.salary_id = salary.id 
 where role_title like '%Senior Java%';
...
select employees.employee_name, salary.monthly_salary, roles.role_title
 ...
 32. Вывести имена, должности и ЗП всех специалистов по возрастанию у специалистов у которых ЗП равна 1100, 1500, 2100
